...
def mesh_less(kappa, prms, mu, g, theta=0.5, M=10, N=10, max_k=20, eps=10):
    ...
    X = np.linspace(0, 1, M+2, dtype=np.float64)[1:-1]
    dt = 1/N
    ...
    for i in range(X.shape[0]):
        for j in range(X.shape[0]):
            ...
            Omega[i, j] = rho(X[i], X[j], eps=eps) + eta*kappa*rho_xx(X[i], X[j], eps=eps) + eta*((mu(X[i], dt, prms)+mu(X[i], 0, prms))/2)*rho_x(X[i], X[j], eps=eps)
    ...
    while n < N:
        ...
        for i in range(X.shape[0]):
            for j in range(X.shape[0]):
                ...
                mu_mean = (mu(X[i], (n+1)*dt, prms) + mu(X[i], (n)*dt, prms))/2

                Omega[i, j] = rho(X[i], X[j], eps=eps) + eta*kappa*rho_xx(X[i], X[j], eps=eps) + eta*mu_mean*rho_x(X[i], X[j], eps=eps)
                Xi[i, j] = rho(X[i], X[j], eps=eps) + dt*theta*kappa*rho_xx(X[i], X[j], eps=eps) + dt*theta*mu_mean*rho_x(X[i], X[j], eps=eps)
